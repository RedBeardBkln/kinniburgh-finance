...
function ConnectPageInner() {
  ...
  // mapping: plaidAccountId → ourAccountId (or "" to skip)
  ...
  // Fetch all seeded accounts for the mapping dropdown
  useEffect(() => {
    fetch("/api/form-data")
      .then((r) => r.json())
  ...
  const onSuccess = useCallback(async (publicToken: string) => {
    ...
    try {
      ...
      // Pre-fill mappings with auto-matched accounts
  ...
  const onExit = useCallback(() => {
    // User closed Plaid Link without completing — stay on link step
  }, []);
    ...
    <AppShell>
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        {step === "link" && (
          <Card>
            <CardHeader>
              <CardTitle>Step 1 — Connect your bank</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">
                Click the button below to open Plaid Link and authenticate with your bank.
                TD Bank and Capital One are fully supported. JCSB, Barclays, and PennyMac
                will be attempted — coverage may vary.
              </p>
              {linkError && (
              ...
              <button
                onClick={() => openLink()}
                disabled={!linkReady || !!linkError || loading}
        ...
        {step === "map" && (
          <Card>
            <CardHeader>
              <CardTitle>Step 2 — Map accounts</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <p className="text-sm text-muted-foreground">
                We auto-matched Plaid accounts to your seeded accounts by last 4 digits.
                Adjust any mismatches, then confirm.
              </p>
                ...
                <tbody>
                  {suggestions.map((s) => (
                    <tr key={s.plaidAccountId} className="border-b last:border-0">
                      ...
                      <td className="py-2">
                        <select
                          value={mappings[s.plaidAccountId] ?? ""}
                          onChange={(e) =>
                            setMappings((m) => ({ ...m, [s.plaidAccountId]: e.target.value }))
                          }
