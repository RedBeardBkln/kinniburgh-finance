...
import { setFaviconMeta } from "@/lib/settings";
...
export async function POST(req: NextRequest) {
  ...
  if (!ext) {
    return NextResponse.json(
      { error: "Unsupported type. Upload PNG, ICO, SVG, or WebP." },
      { status: 400 }
    );
  }
  ...
  await setFaviconMeta(fileKey, file.type);
